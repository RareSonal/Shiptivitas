...
export default class Board extends Component {
  ...
  async updateCardStatus(el, target, sibling) {
    ...
    try {
      const response = await fetch(`${API}/api/v1/cards/${card.id}`, {
        method: 'PUT',
      ...
      // Log card change
      this.props.onCardChange(card.id, oldStatus, targetStatus, oldPriority, newPriority);
    } catch (err) {
  ...
  updateCardsState(updatedCards) {
    const newCards = { backlog: [], inProgress: [], complete: [] };
  ...
  renderSwimlane(name, status, cards, ref) {
    return (
      <Swimlane
        name={name}
        status={status}
        clients={cards.map(c => ({ ...c, style: this.getCardStyle(c.status) }))}
        dragulaRef={ref}
      />
    );
  }
